...
namespace ItextPDF {
    ...
    public class ResizePDFGiraudi {
        ...
        public static Font font_FOOTERSPECIAL;

        static ResizePDFGiraudi()
        { 
            font = BaseFont.CreateFont(BaseFont.HELVETICA, BaseFont.CP1252, false);
            //fontSPECIAL= BaseFont.CreateFont(BaseFont.HELVETICA, BaseFont.CP1252, false);
            font_FOOTER = new Font(font, 6, Font.NORMAL, BaseColor.BLACK);
            font_FOOTERSPECIAL = FontFactory.GetFont("Calibri", 10, Font.BOLD, BaseColor.BLACK);
        }
        ...
        public class PDFFooter : PdfPageEventHelper
        {
            ...
            public override void OnEndPage(PdfWriter writer, Document document)
            {
                ...
                if (caption_data.Length > 0)
                {
                    cs = caption_data[0].Contains("#CS#");
                }
                ...
                if (cs == true)
                {
                    ...
                    {
                        includeFirma(writer, document, cs);
                    }
                }
                else
                {
                ...
                tabFot.TotalWidth = document.PageSize.Width - 300;

                if (cs)
                {
                    if (document.PageNumber == 1)
                    {
                        tabFot.TotalWidth = document.PageSize.Width - 240;
                    }
                    else
                    {
                        tabFot.TotalWidth = document.PageSize.Width - 120;
                    }
                }
                ...
                foreach (var pair in caption_data)
                {
                    ...
                    if (document.PageNumber == 1)
                    {
                        ...
                        if (cs)
                        {
                            xCell = new PdfPCell(new Phrase(new Chunk(pair.ToString().Replace("#P01#", "").Replace("#CS#", ""), font_FOOTERSPECIAL)));
                        }
                        else
                        {
                            xCell = new PdfPCell(new Phrase(new Chunk(pair.ToString().Replace("#P01#", "").Replace("#CS#", ""), font_FOOTER)));
                        }
                    ...
                    {
                        if (!pair.ToString().StartsWith("#P01#"))
                        {
                            PdfPCell xCell;
                ...
                if (!cs)
                {
                    tabFot.WriteSelectedRows(0, -1, 160, document.Bottom + 5, writer.DirectContent);
                }
                else
                {
                    tabFot.WriteSelectedRows(0, -1, 60, document.Bottom + 5, writer.DirectContent);
                }
            }
        ...
        public static void Create(FileStream output) {
            ...
            Document document = new Document(PageSize.A4, 2, 2, 10, 60);
            ...
            try {
                ...
                includePDFScaled(writer, document, 0.8f);
        ...
        public static void includeFirma(PdfWriter writer, Document doc, bool special = false)
        {
            //Test files that we'll be creating
            if (File.Exists(fileNameFirmaCliente))
            {
                ...
                if (File.Exists(fileImg))
                {
                    ...
                    if (special)
                    {
                        xCell = new PdfPCell(new Phrase(new Chunk(drc, font_FOOTERSPECIAL)));
                    }
                    else
                    {
                        xCell = new PdfPCell(new Phrase(new Chunk(drc, font_FOOTER)));
                    }
                    ...
                    xCellImg.VerticalAlignment = PdfPCell.ALIGN_TOP; // ALIGN_CENTER;
                    xCellImg.Padding = 1;
                    xCellImg.FixedHeight = xCell.Height * 3;

                    tabFot.AddCell(xCellImg);

                    if (special)
                    {
                        tabFot.WriteSelectedRows(0, -1, 420, doc.Bottom + 5, writer.DirectContent);
                    }
                    else
                    {
                        tabFot.WriteSelectedRows(0, -1, 460, doc.Bottom + 5, writer.DirectContent);
                    }
